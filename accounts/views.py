...
from .spoonacular_service import spoonacular_service
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if spoonacular_service.is_configured():
                nutrition_data = spoonacular_service.analyze_ingredients(
                    meal.ingredients,
                    servings=1
                )
                
                if nutrition_data:
                    meal.calories = nutrition_data.get('calories')
                    meal.protein = nutrition_data.get('protein')
                    meal.carbs = nutrition_data.get('carbs')
                    meal.fat = nutrition_data.get('fat')
                    meal.allergens = ', '.join(nutrition_data.get('allergens', []))
                    
                    # Auto-suggest dietary tag if not manually set
                    if meal.dietary_tags == 'none':
                        suggested_tag = spoonacular_service.suggest_dietary_tag(nutrition_data)
                        meal.dietary_tags = suggested_tag
                    
                    meal.nutrition_info = 'Nutrition info auto-generated by Spoonacular API'
                    messages.success(request, 'Meal listing created with auto-generated nutrition info!')
                else:
                    meal.nutrition_info = 'Nutrition info could not be generated (API limit or error)'
                    messages.warning(request, 'Meal created, but nutrition info could not be generated.')
            else:
                meal.nutrition_info = 'Spoonacular API not configured'
                messages.info(request, 'Meal created. Configure Spoonacular API for auto nutrition info.')
...
def edit_meal(request, meal_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Re-fetch nutrition info if ingredients changed
            if 'ingredients' in form.changed_data and spoonacular_service.is_configured():
                nutrition_data = spoonacular_service.analyze_ingredients(
                    updated_meal.ingredients,
                    servings=1
                )
                
                if nutrition_data:
                    updated_meal.calories = nutrition_data.get('calories')
                    updated_meal.protein = nutrition_data.get('protein')
                    updated_meal.carbs = nutrition_data.get('carbs')
                    updated_meal.fat = nutrition_data.get('fat')
                    updated_meal.allergens = ', '.join(nutrition_data.get('allergens', []))
                    
                    # Auto-suggest dietary tag if changed to 'none'
                    if updated_meal.dietary_tags == 'none':
                        suggested_tag = spoonacular_service.suggest_dietary_tag(nutrition_data)
                        updated_meal.dietary_tags = suggested_tag
                    
                    updated_meal.nutrition_info = 'Nutrition info auto-updated by Spoonacular API'
                    messages.success(request, 'Meal updated with refreshed nutrition info!')
                else:
                    messages.warning(request, 'Meal updated, but nutrition info could not be refreshed.')
            else:
